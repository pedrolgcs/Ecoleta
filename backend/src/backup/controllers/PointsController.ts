...
import ApppError from '../utils/errors/AppError';
...
class PointsController {
  ...
  public async create(request: Request, response: Response): Promise<Response> {
    ...
    if (!items) {
      throw new ApppError('Items is a required field', 400);
    }
    ...
    return response.status(201).json({ id: insertedIds, ...point });
  }
  ...
  public async show(request: Request, response: Response): Promise<Response> {
    ...
    if (!point) {
      throw new ApppError('Point not found', 400);
    }
